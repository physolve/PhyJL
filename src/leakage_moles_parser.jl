...
# Constants
Cv = 0.00085*1.1 * 1.7e-5  # Convert Cv to SI units (m³/s·Pa^0.5)
gamma = 1.41
M = 2.016e-3  # kg/mol
R = 8.314
T = 298
...
for t in 0:dt:t_final
    ...
    if delta_P > 0
        ...
        if Pdown / Pup < (2 / (gamma + 1))^(gamma / (gamma - 1))
            # Choked flow
            dp_coef = 0.0375*delta_P*1e-5+1
            m_dot = Cv * dp_coef * Pup * sqrt((gamma * M) / (R * T) * (2 / (gamma + 1)) ^ ((gamma + 1) / (gamma - 1)))
        else
            # Subsonic flow
            dp_coef = 0.00455*delta_P*1e-5+0.66451
            term = (Pdown / Pup) ^ (2 / gamma) - (Pdown / Pup) ^ ((gamma + 1) / gamma)
            m_dot = Cv * dp_coef * Pup * sqrt((2 * gamma * M) / ((gamma - 1) * R * T) * term)
        end
    else
        # No flow (delta_P <= 0)
        m_dot = 0
...
file_model="2025-04-09_R0_$(runT)_model.txt"
